...
<u>Note:</u> If using the "contigs" option with a combined set of reference genomes make sure the names of the contigs in the final FASTA file are structured as [genome_name]_1, [genome_name]_2, etc. You can use the `rename_multifasta_prefix.py` script here provided in the `tools/` folder to rename each genome FASTA before concatenating the files into one.
